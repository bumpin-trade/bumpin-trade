...
export class PollingPoolAccountSubscriber implements AccountSubscriber<Pool> {
  ...
  private printPool(pool_data: DataAndSlot<Pool>) {
    ...
    console.log(`Insurance Fund Amount: ${pool.insuranceFundAmount.toString()}`);
    ...
    console.log('Balance:');
    console.log(`  Settle Funding Fee: ${pool.balance.settleFundingFee.toString()}`);
    ...
    console.log(`  Settle Funding Fee Amount: ${pool.balance.settleFundingFeeAmount.toString()}`);
    console.log(`  Loss Amount: ${pool.balance.lossAmount.toString()}`);

    console.log('Stable Balance:');
    console.log(`  Settle Funding Fee: ${pool.stableBalance.settleFundingFee.toString()}`);
    console.log(`  Amount: ${pool.stableBalance.amount.toString()}`);
    console.log(`  Hold Amount: ${pool.stableBalance.holdAmount.toString()}`);
    console.log(`  Unsettle Amount: ${pool.stableBalance.unSettleAmount.toString()}`);
    console.log(`  Settle Funding Fee Amount: ${pool.stableBalance.settleFundingFeeAmount.toString()}`);
    console.log(`  Loss Amount: ${pool.stableBalance.lossAmount.toString()}`);

    console.log('Borrowing Fee:');
    console.log(`  Total Borrowing Fee: ${pool.borrowingFee.totalBorrowingFee.toString()}`);
    console.log(`  Total Realized Borrowing Fee: ${pool.borrowingFee.totalRealizedBorrowingFee.toString()}`);
    console.log(`  Cumulative Borrowing Fee Per Token: ${pool.borrowingFee.cumulativeBorrowingFeePerToken.toString()}`);
    ...
    console.log(`  Unsettle Fee Amount: ${pool.feeReward.unSettleFeeAmount.toString()}`);
    console.log(`  Cumulative Rewards Per Stake Token: ${pool.feeReward.cumulativeRewardsPerStakeToken.toString()}`);
    console.log(`  Last Rewards Per Stake Token Deltas: ${pool.feeReward.lastRewardsPerStakeTokenDeltas.map(delta => delta.toString()).join(', ')}`);

    console.log('Stable Fee Reward:');
    console.log(`  Fee Amount: ${pool.stableFeeReward.feeAmount.toString()}`);
    console.log(`  Unsettle Fee Amount: ${pool.stableFeeReward.unSettleFeeAmount.toString()}`);
    console.log(`  Cumulative Rewards Per Stake Token: ${pool.stableFeeReward.cumulativeRewardsPerStakeToken.toString()}`);
    console.log(`  Last Rewards Per Stake Token Deltas: ${pool.stableFeeReward.lastRewardsPerStakeTokenDeltas.map(delta => delta.toString()).join(', ')}`);

    console.log('Config:');
    console.log(`  Minimum Stake Amount: ${pool.config.minimumStakeAmount.toString()}`);
    console.log(`  Minimum UnStake Amount: ${pool.config.minimumUnStakeAmount.toString()}`);
    console.log(`  Pool Liquidity Limit: ${pool.config.poolLiquidityLimit.toString()}`);
    console.log(`  Borrowing Interest Rate: ${pool.config.borrowingInterestRate.toString()}`);
    console.log(`  Stake Fee Rate: ${pool.config.stakeFeeRate}`);
    console.log(`  UnStake Fee Rate: ${pool.config.unStakeFeeRate}`);
    console.log(`  UnSettle Mint Ratio Limit: ${pool.config.unSettleMintRatioLimit}`);
