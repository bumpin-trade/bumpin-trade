...
export class BulkAccountLoader {
    ...
    public async addAccount(
        ...
        const existingAccountToLoad = this.accountsToLoad.get(
            publicKey.toString(),
        );
    ...
    async loadChunk(accountsToLoadChunks: AccountToLoad[][]): Promise<void> {
        ...
        rpcResponses.forEach(
            (
                ...
                accountsToLoad.forEach(
                    (
                        accountToLoad: { callbacks: any; publicKey: any },
                        j: number,
                    ) => {
                        ...
                        if (rpcResponse.result.value[j]) {
                            const raw: string =
                                rpcResponse.result.value[j].data[0];
                            const dataType =
                                rpcResponse.result.value[j].data[1];
                        ...
                        if (!oldRPCResponse) {
                            ...
                            if (!newBuffer) {
                                throw new BumpinInvalidParameter(
                                    'newBuffer is undefined',
                                );
                            }
                            this.handleAccountCallbacks(
                                accountToLoad,
                                newBuffer,
                                newSlot,
                            );
                        ...
                        if (
                            newBuffer &&
                            (!oldBuffer || !newBuffer.equals(oldBuffer))
                        ) {
                            this.bufferAndSlotMap.set(key, {
                                slot: newSlot,
                                buffer: newBuffer,
    ...
    handleAccountCallbacks(
        ...
        for (const [_, callback] of accountToLoad.callbacks) {
            try {
                ...
                console.log(
                    'accounto to load',
                    accountToLoad.publicKey.toString(),
                );
