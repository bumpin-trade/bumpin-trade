...
export class BulkAccountLoader {
    ...
    async loadChunk(accountsToLoadChunks: AccountToLoad[][]): Promise<void> {
        ...
        rpcResponses.forEach((rpcResponse: {
            result: { context: { slot: any; }; value: { data: any[]; }[]; };
        }, i: string | number) => {
            ...
            accountsToLoad.forEach((accountToLoad: { callbacks: any; publicKey: any; }, j: number) => {
                ...
                if (!oldRPCResponse) {
                    ...
                    if (!newBuffer) {
                        throw new BumpinInvalidParameter('newBuffer is undefined');
                    }
