...
export class BumpinUserUtils {
    ...
    public static getMyTokenByMint(me: User, mint: PublicKey): UserToken | undefined {
        let userToken = me.tokens.find((token) => {
            return isEqual(token.userTokenStatus, UserTokenStatus.USING) && token.tokenMintKey.equals(mint);
        });
        return userToken;
    }
