...
export class BumpinClient {
    ...
    public async getPoolNetPrice(poolKey: PublicKey, sync: boolean = false) {
        ...
        if (pool.totalSupply.isZero()) {
            ...
            return poolValueUsd.div(
                pool.totalSupply.toBigNumber().div(BigNumber(10).pow(BigNumber(tradeToken.decimals)))
            );
        }
    }
    ...
    public async getUserAvailableValue(
        ...
        return BumpinUtils.amount2Size(await this.userComponent!.getUserAvailableValue(user, await this.getTradeTokens(), await this.getMarkets(), await this.getPools()), 8);
    }
