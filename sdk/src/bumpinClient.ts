...
import {
    ...
    UserStakeStatusAccount, UserSummary, UserTokenSummary,
    WalletBalance,
} from "./typedef";
import {
...
export class BumpinClient {
    ...
    public async getUserSummary(sync: boolean = false): Promise<UserSummary> {
        ...
        let accountNetValue = await this.userComponent!.getUserAccountNetValue(user, tradeTokens, markets, pools);
        ...
        for (let tradeToken of tradeTokens) {
            ...
            let userToken = BumpinUserUtils.getMyTokenByMint(user, tradeToken.mintKey);
            if (userToken) {
                userTokenSummary.amount = userToken.amount;
                userTokenSummary.used = userToken.usedAmount;
                userTokenSummary.borrow = userToken.usedAmount.gt(userToken.amount) ? userToken.usedAmount.minus(userToken.amount).minus(userToken.liabilityAmount) : new BigNumber(0);
            }
    ...
    public async getPoolValueUsd(
        ...
        if (!pool.stable) {
            ...
            if (stableAmount.gt(BigNumber(0))) {
                ...
                rawValue = rawValue.plus(stableAmount.multipliedBy(stablePrice.price));
            }
        }
    ...
    public async getMarketUnPnlUsd(market: Market): Promise<MarketUnPnlUsd> {
        ...
        if (!longPosition.entryPrice.isZero()) {
            longUnPnl = longPosition.openInterest
                .multipliedBy(BigNumber(price.price).minus(longPosition.entryPrice))
        ...
        for (const stake of user.stakes) {
            if (
                ...
                claimResult.claimed = claimResult.claimed.plus(
                    stake.userRewards.totalClaimRewardsAmount.multipliedBy(price)
                );
