...
impl PositionProcessor<'_> {
    ...
    pub fn get_liquidation_price(&mut self, market: &Market, pool: &Pool, state: &State, oracle_map: &mut OracleMap) -> BumpResult<u128> {
        ...
        let position_value = if self.position.is_long {
            position_fee_usd.safe_add(self.position.position_size.safe_sub(self.position.initial_margin_usd)?.safe_add(mm_usd)?.cast()?)?
        } else {
            self.position.position_size.safe_add(self.position.initial_margin_usd)?
                .safe_sub(mm_usd)?.cast::<i128>()?
                .safe_sub(position_fee_usd)?
        };
        if position_value < 0 {
            Ok(0)
        } else {
            let liquidation_price = position_value.cast::<u128>()?
    ...
    pub fn get_position_mm(&self, market: &Market, state: &State) -> BumpResult<u128> {
        Ok(self.get_mm(self.position.position_size, market.market_trade_config.max_leverage, state.max_maintenance_margin_rate)?)
    }
    pub fn get_position_fee(&self, market: &Market, pool: &Pool, oracle_map: &mut OracleMap) -> BumpResult<i128> {
        ...
        let funding_fee_amount_per_size = if self.position.is_long { market.funding_fee.long_funding_fee_amount_per_size } else { market.funding_fee.short_funding_fee_amount_per_size };
        let funding_fee = cal_utils::mul_small_rate_i(self.position.position_size.cast::<i128>()?,
                                                      funding_fee_amount_per_size.safe_sub(self.position.open_funding_fee_amount_per_size)?)?;
        let price_data = oracle_map.get_price_data(&self.position.margin_mint)?;

        if self.position.is_long {
            let funding_fee_usd = cal_utils::token_to_usd_i(funding_fee, 8u8, price_data.price)?;
        ...
        let initial_margin_leverage = cal_utils::mul_small_rate_u(self.position.initial_margin, self.position.leverage.safe_sub(RATE_PRECISION)?)?;
        let borrowing_fee = cal_utils::mul_small_rate_u(pool.borrowing_fee.cumulative_borrowing_fee_per_token.safe_sub(self.position.open_borrowing_fee_per_token)?, initial_margin_leverage)?;
        borrowing_fee_total_usd = borrowing_fee_total_usd.safe_add(borrowing_fee.safe_mul(price_data.price)?)?;
        Ok(funding_fee_total_usd.safe_add(borrowing_fee_total_usd.cast()?)?.safe_add(self.position.close_fee_in_usd.cast()?)?)
    }
    ...
    pub fn increase_position(&mut self,
        ...
        let increase_margin_from_balance = if params.increase_margin_from_balance > fee { cal_utils::sub_u128(params.increase_margin_from_balance, fee)? } else { 0u128 };
        let decimal = trade_token.decimals;
        let increase_size = cal_utils::token_to_usd_u(cal_utils::mul_u128(increase_margin, params.leverage)?, decimal, params.margin_token_price)?;
        ...
        let increase_hold = cal_utils::mul_rate_u(increase_margin, cal_utils::sub_u128(params.leverage, 1u128)?)?;
        self.position.add_hold_pool_amount(increase_hold)?;


        // update market io
        market_processor.update_oi(true, UpdateOIParams {
    ...
    pub fn decrease_position<'info>(&mut self,
        ...
        if params.decrease_size == self.position.position_size {
            ...
            self.position.sub_initial_margin_usd_from_portfolio(response.decrease_margin_in_usd_from_portfolio)?;
    ...
    pub fn execute_reduce_position_margin(&mut self,
        ...
        if self.position.cross_margin && self.position.initial_margin_usd.safe_sub(self.position.initial_margin_usd_from_portfolio)? < reduce_margin_amount {
            self.position.sub_initial_margin_usd_from_portfolio(reduce_margin_amount.safe_sub(self.position.initial_margin_usd.safe_sub(self.position.initial_margin_usd_from_portfolio)?)?.max(0u128))?;
        }
        ...
        if !self.position.cross_margin {
            self.position.set_initial_margin_usd_from_portfolio(self.position.initial_margin_usd)?;
        }
    ...
    fn update_decrease_position(&mut self,
                                ...
                                market: &Market,
                                state: &State,
                                trade_token: &TradeToken) -> BumpResult<UpdateDecreaseResponse> {
        ...
        response.decrease_margin = self.position.initial_margin
            .safe_mul(decrease_size)?
            .safe_div(self.position.position_size)?;
        response.decrease_margin_in_usd = self.position.initial_margin_usd
            .safe_mul(decrease_size)?
            .safe_div(self.position.position_size)?;
        response.un_hold_pool_amount = self.position.hold_pool_amount
            .safe_mul(decrease_size)?
            .safe_div(self.position.position_size)?;
        ...
        if self.position.position_size == decrease_size && is_liquidation {
            response.settle_margin = if is_cross_margin {
                ...
                cal_utils::usd_to_token_i(self.position.initial_margin_usd.cast::<i128>()?
                                              .safe_sub(self.get_pos_fee_in_usd(settle_funding_fee_in_usd, settle_borrowing_fee_in_usd, settle_close_fee_in_usd)?)?
                                              .safe_add(pnl)?
            ...
            response.settle_margin = cal_utils::usd_to_token_i(self.position.initial_margin_usd.cast::<i128>()?
                                                                   .safe_add(pnl)?
    ...
    fn get_pos_fee_in_usd(&self, funding_fee_in_usd: i128, borrowing_fee_in_usd: u128, close_fee_in_usd: u128) -> BumpResult<i128> {
        Ok(
            funding_fee_in_usd
    ...
    fn cal_decrease_close_fee(&self, decrease_size: u128, trade_token: &TradeToken, token_price: u128, close_fee_rate: u128) -> BumpResult<(u128, u128)> {
        ...
        return Ok((cal_utils::usd_to_token_u(close_fee_in_usd, trade_token.decimals, token_price)?
                       .safe_mul(decrease_size)?
                       .safe_div(self.position.position_size)?,
                   close_fee_in_usd.safe_mul(decrease_size)?
                       .safe_div(self.position.position_size)?));
    }


    fn update_borrowing_fee(&mut self, pool: &mut Pool, token_price: u128, token: &TradeToken) -> BumpResult<()> {
        pool.borrowing_fee.cumulative_borrowing_fee_per_token;
        let realized_borrowing_fee = self.position.initial_margin
            .safe_mul(self.position.leverage)?
            .safe_mul(pool.borrowing_fee.cumulative_borrowing_fee_per_token.safe_sub(self.position.open_borrowing_fee_per_token)?)?;

        self.position.add_realized_borrowing_fee(realized_borrowing_fee)?;
        self.position.add_realized_borrowing_fee_in_usd(cal_utils::token_to_usd_u(realized_borrowing_fee, token.decimals, token_price)?)?;
        self.position.set_open_borrowing_fee_per_token(pool.borrowing_fee.cumulative_borrowing_fee_per_token)?;
    ...
    fn update_funding_fee(&mut self, mut market: &mut Market, token_price: u128, token: &TradeToken) -> BumpResult<()> {
        ...
        let realized_funding_fee = cal_utils::mul_small_rate_i(self.position.position_size.cast::<i128>()?, market_funding_fee_per_size.cast::<i128>()?.safe_sub(self.position.open_funding_fee_amount_per_size.cast::<i128>()?)?)?;
    ...
    fn add_insurance_fund(&mut self, market: &Market, state: &State, trade_token: &TradeToken, response: &UpdateDecreaseResponse, pool: &mut Pool) -> BumpResult<()> {
        ...
        if response.settle_fee >= 0i128 {
            ...
            add_funds = response.decrease_margin
    ...
    fn settle<'info>(&mut self,
        ...
        if self.position.cross_margin {
            ...
            pool_processor.update_pnl_and_un_hold_pool_amount(response.un_hold_pool_amount, response.pool_pnl_token, add_liability, Some(base_token_pool))?;
        } else {
            ...
            let base_token_pool = &mut pool_account_loader.load_mut().unwrap();
            pool_processor.update_pnl_and_un_hold_pool_amount(response.un_hold_pool_amount, response.pool_pnl_token, 0u128, Some(base_token_pool))?;
        }
        Ok(())
    }
    ...
    fn settle_cross<'info>(&mut self,
        ...
        if !response.is_liquidation {
            let change_token_amount = response.decrease_margin_in_usd_from_portfolio
    ...
    fn update_all_position_from_portfolio_margin(&self, user: &mut User, change_token_amount: i128, token_mint: &Pubkey) -> BumpResult<()> {
        ...
        for mut position in user.user_positions {
            if position.margin_mint.eq(token_mint) && position.cross_margin {
                ...
                reduce_amount = if change_token_amount > 0i128 { reduce_amount.cast::<i128>()?.safe_sub(change_amount.cast::<i128>()?)?.cast::<i128>()? } else {
                    reduce_amount.cast::<i128>()?.safe_add(change_amount.cast::<i128>()?)?.cast::<i128>()?
                };
    ...
    pub fn get_position_un_pnl_usd(&self, index_price: u128) -> BumpResult<i128> {
        ...
        if self.position.is_long {
            Ok(self.position.position_size.cast::<i128>()?
                .safe_mul(index_price.cast::<i128>()?
                    .safe_sub(self.position.entry_price.cast::<i128>()?)?)?
                .safe_div(self.position.entry_price.cast::<i128>()?)?)
        } else {
            Ok(self.position.position_size.cast::<i128>()?
                .safe_mul(self.position.entry_price.cast::<i128>()?
                    .safe_sub(index_price.cast::<i128>()?)?)?
                .safe_div(self.position.entry_price.cast::<i128>()?)?)
        }
    }

    pub fn get_position_un_pnl_token(&self, trade_token: &TradeToken, mint_token_price: u128, index_price: u128) -> BumpResult<i128> {
        if self.position.position_size == 0u128 {
            return Ok(0i128);
        };
