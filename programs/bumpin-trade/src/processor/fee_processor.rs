...
pub fn collect_stake_fee(stake_pool: &mut Pool, state: &mut State, amount: u128) -> BumpResult<u128> {
...
pub fn collect_un_stake_fee(stake_pool: &mut Pool, state: &mut State, un_stake_amount: u128) -> BumpResult<u128> {
...
pub fn collect_open_position_fee(market: &Market, stake_pool: &mut Pool, state: &mut State, margin: u128, cross_margin: bool) -> BumpResult<u128> {
...
pub fn collect_close_position_fee(stake_pool: &mut Pool, state: &mut State, fee_amount: u128, cross_margin: bool) -> BumpResult<u128> {
    let pool_rewards_fee = fee_amount.safe_mul(state.trading_fee_pool_rewards_ratio)?;
    let staking_rewards_fee = fee_amount.safe_mul(state.trading_fee_staking_rewards_ratio)?;
    let dao_rewards_fee = fee_amount.safe_sub(pool_rewards_fee)?.safe_sub(staking_rewards_fee)?;
...
pub fn collect_borrowing_fee(stake_pool: &mut Pool, state: &mut State, fee_amount: u128, cross_margin: bool) -> BumpResult<()> {
