...
pub fn collect_short_open_position_fee(
    ...
    if let Some(ref mut stable_pool) = stable_pool {
        ...
        pool.fee_reward.add_fee_amount(usd_pool_rewards_fee)?;
    }
    ...
    if is_portfolio_margin {
        ...
        if let Some(ref mut stable_pool) = stable_pool {
            ...
            pool.fee_reward.add_un_settle_amount(usd_pool_rewards_fee)?;
        }
    }
...
pub fn settle_funding_fee(
    ...
    if !is_long {
        if fee_amount_usd <= 0i128 {
            ...
            if let Some(stable_token_pool) = stable_pool {
                stable_token_pool.add_unsettle(fee_amount_usd.cast::<u128>()?)?;
            } else {
                base_token_pool.add_unsettle(fee_amount_usd.cast::<u128>()?)?;
            }
        } else if is_cross {
            if let Some(stable_token_pool) = stable_pool {
                stable_token_pool.add_unsettle(fee_amount_usd.cast::<u128>()?)?;
