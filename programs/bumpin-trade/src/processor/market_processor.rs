...
impl<'a> MarketProcessor<'_> {
    pub fn get_market_un_pnl(&mut self, _is_long: bool, _oracle_map: &OracleMap) -> BumpResult<u128> {
        //todo
        Ok(0u128)
    }
    ...
    fn add_oi(&self, params: UpdateOIParams) -> BumpResult<()> {
        let mut market_position = if params.is_long {
            self.market.long_open_interest
        } else {
