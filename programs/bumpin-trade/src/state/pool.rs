...
impl Pool {
    ...
    pub fn add_stable_balance_unsettle(&mut self, amount: u128) -> BumpResult<()> {
        self.stable_balance.un_settle_amount = add_u128(self.stable_balance.un_settle_amount, amount)?;
        Ok(())
    }
        ...
        Ok(match base_token_pool {
            ...
            Some(base_token_pool) => {
                ...
                if token_pnl < 0i128 {
                    ...
                    base_token_pool.add_stable_balance_unsettle(if u_token_pnl > add_liability {
                        add_liability
                    } else {
                        u_token_pnl
                    })?;
