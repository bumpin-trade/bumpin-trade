...
pub fn handle_liquidate_cross_position(ctx: Context<LiquidateCrossPosition>, user: Pubkey) {
    ...
    let (total_im_usd, total_un_pnl_usd, total_position_fee, total_position_mm) = user_processor.get_user_cross_position_value(&state, &market_map, &pool_map, &mut oracle_map)?;
