...
pub fn handle_liquidate_cross_position<'a, 'b, 'c: 'info, 'info>(
    ...
    let AccountMaps { market_map, trade_token_map, mut oracle_map, pool_map, .. } =
    ...
    let (total_im_usd, total_un_pnl_usd, total_position_fee, total_position_mm) = user_processor
        .get_user_cross_position_value(
            &ctx.accounts.state,
            &market_map,
            &pool_map,
            &mut oracle_map,
