...
pub fn usd_to_token_u(usd_value: u128, decimals: u8, token_price: u128) -> BumpResult<u128> {
    mul_div_u(usd_value, 10 * *decimals.cast::<u128>()?, token_price.safe_mul(PRICE_TO_LAMPORT)?)
}

pub fn usd_to_token_i(usd_value: i128, decimals: u8, token_price: u128) -> BumpResult<i128> {
    mul_div_i(usd_value, 10 * *decimals.cast::<i128>()?, token_price.cast::<i128>()?.safe_mul(PRICE_TO_LAMPORT.cast::<i128>()?)?)
}

pub fn token_to_usd_u(token_amount: u128, decimals: u8, token_price: u128) -> BumpResult<u128> {
    token_amount.safe_mul(token_price)?.safe_mul(PRICE_TO_LAMPORT)?.safe_div(10 * *decimals.cast::<u128>()?)?
}

pub fn token_to_usd_i(token_amount: i128, decimals: u8, token_price: u128) -> BumpResult<i128> {
    token_amount.safe_mul(token_price.cast::<i128>()?)?.safe_mul(PRICE_TO_LAMPORT.cast::<i128>()?)?.safe_div(10 * *decimals.cast::<i128>()?)?
}


pub fn current_time() -> u128 {
    let clock = Clock::get()?;
    clock.unix_timestamp.to_u128()?
}
